...
// ═══════════════════════════════════════════════════════════════════════════════
// 
// This is the DEDICATED OUTCOMES IMAGE CREATION SCREEN with its OWN SPECIFIC UI
// 
// OUTCOMES HAS DIFFERENT FEATURES THAN LIBRARY:
// - Image picker with gallery/camera
...
// 
// ⛔ DO NOT:
// - Consolidate this with RichTextEditor or ImageCreationScreen
...
class OutcomeImageCreationScreen extends StatefulWidget {
  final String? itemId; // For editing existing items

  const OutcomeImageCreationScreen({
    super.key,
    this.itemId,
  });

  @override
  State<OutcomeImageCreationScreen> createState() => _OutcomeImageCreationScreenState();
}

class _OutcomeImageCreationScreenState extends State<OutcomeImageCreationScreen> {
  ...
  Future<void> _loadExistingItem() async {
    ...
    if (item != null) {
      setState(() {
        ...
        _savedImagePath = item.rawTranscript; // Image path stored in rawTranscript
    ...
    try {
      ...
      if (widget.itemId != null) {
        ...
        final newItem = RecordingItem(
          id: const Uuid().v4(),
          rawTranscript: _savedImagePath ?? '', // Store image path
          finalText: _captionController.text.trim(),
          presetUsed: '${_selectedOutcomeType!.displayName} Image',
          outcomes: [_selectedOutcomeType!.toStorageString()],
          projectId: null, // Outcomes don't belong to projects
          createdAt: DateTime.now(),
          editHistory: [_captionController.text.trim()],
          presetId: '${_selectedOutcomeType!.toStorageString()}_image',
          tags: _selectedTags,
          customTitle: _titleController.text.trim().isEmpty ? null : _titleController.text.trim(),
          contentType: 'image',
          reminderDateTime: _reminderDateTime,
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(widget.itemId != null ? 'Image outcome updated' : 'Image outcome created'),
    ...
    return WillPopScope(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          title: Text(
            widget.itemId != null ? 'Edit Image Outcome' : 'Create Image Outcome',
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                // Tags, Reminder, and Completion status
                Row(
                  children: [
                    // Tags display
                ...
                if (_reminderDateTime != null)
                  Container(
                    ...
                    child: Row(
                      children: [
                        Icon(Icons.alarm, color: primaryColor, size: 14),
                        ...
                        GestureDetector(
                          ...
                          child: Icon(Icons.close, color: primaryColor, size: 14),
                        ),
                      ],
                    ),
                  ),
                ...
                Expanded(
                  child: Container(
                    ...
                    child: TextField(
                      ...
                      decoration: InputDecoration(
                        hintText: _selectedOutcomeType != null 
                            ? 'Add a caption for your ${_selectedOutcomeType!.displayName.toLowerCase()}...'
                ...
                Row(
                  ...
                  children: [
                    Text(
                      (_selectedImage != null || _savedImagePath != null) ? 'Image selected' : 'No image selected',
                      style: TextStyle(
                        color: (_selectedImage != null || _savedImagePath != null) ? const Color(0xFF10B981) : secondaryTextColor,
                        fontSize: 12,
                      ),
                    ),
                    if (_hasUnsavedChanges)
